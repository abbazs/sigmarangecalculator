...
class hdf5db(object, metaclass=signleton):
    ...
    def set_db(self, dbpath):
        try:
            if self.dbpath is None:
                ...
                print(f'dbpath already set to {self.dbpath}')
    ...
    @classmethod
    def get_past_n_expiry_dates(cls, symbol, instrument, n):
        try:
            s = symbol.upper()
            i = instrument.upper()
            cd = dutil.get_current_date()
            df = pd.read_hdf(cls().dbpath, 'fno', 
    ...
    @classmethod
    def get_next_expiry_dates(cls, symbol, instrument):
        try:
            s = symbol.upper()
            i = instrument.upper()
            cd = dutil.get_current_date()
    ...
    @classmethod
    def get_expiry_dates_on_date(cls, symbol, instrument, date):
        try:
            s = symbol.upper()
            i = instrument.upper()
            cd = hdf5db.process_date(date)
            dbp = cls().dbpath
    ...
    @classmethod
    def get_index_data_between_dates(cls, symbol, st, nd):
        try:
            s = symbol.upper()
            std = dutil.process_date(st)
            ndd = dutil.process_date(nd)
            std, ndd = dutil.fix_start_and_end_date(std, ndd)
            dbp = cls().dbpath
            df = pd.read_hdf(dbp, 'idx', 
                            where='SYMBOL==s and TIMESTAMP>=std and TIMESTAMP>=ndd').sort_values('TIMESTAMP')
            df = df.drop_duplicates()
            df = f.set_index('TIMESTAMP')
            return df
        except Exception as e:
            print_exception(e)
    
    @classmethod
    def get_index_data_for_last_n_days(cls, symbol, n_days):
        try:
            ...
            spot_data = cls().get_spot_data_between_dates(symbol, start_date, end_date)
